...
// [[Rcpp::export]]
void g4mMai(std::string INFILE, std::string OUTFILE) {
  g4m::mai mai;
...
//lc8er4 woody savannas-Boreal
  ...
  while(getline(fIn, line)) {
    ...
    for(int i=0; i<12; ++i) {p[i] = atof(tokens[i+8].c_str());}
    std::valarray<double> t(12);
    for(int i=0; i<12; ++i) {t[i] = atof(tokens[i+20].c_str());}
    std::valarray<double> r(12);
    for(int i=0; i<12; ++i) {r[i] = atof(tokens[i+32].c_str());}

    mai.setTemperature(t);
    mai.setPrecipitation(p);
    mai.setRadiation(r);
    
    mai.setAltitude(atof(tokens[2].c_str()));
    mai.setCo2(atof(tokens[44].c_str()));
    mai.setP(atof(tokens[3].c_str()));
    mai.setWhc(atof(tokens[4].c_str()));
    mai.setN(atof(tokens[5].c_str()));
    mai.setpH(atof(tokens[6].c_str()));
    mai.setS(atof(tokens[7].c_str()));
    
    valarray<double> rmai = mai.getMai(mai.testBoundaries(), true);
    fOut << tokens[0] << " " << tokens[1] << " " << rmai.max() << endl;
  }
  fOut.close();
  fIn.close();
}
